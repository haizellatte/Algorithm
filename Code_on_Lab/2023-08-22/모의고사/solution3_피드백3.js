...
function returnResult(max, value1, value2, value3) {
  return [value1, value2, value3].reduce((acc, x, i) => {
    if (x === max) {
      acc.push(i + 1);
    }
    return acc;
...
function solution(answers) {
  ...
  const grade = pArr.map(arr => grading(answers, arr));
  const max = Math.max(...grade);

  return returnResult(max, ...grade);
}
