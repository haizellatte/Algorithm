function solution(clothes) {
    ...
    for(let i = 0 ; i < clothes.length ; i++){
        ...
        //1-3-1. 이미 저장되어 있다면 -> 그 value를 꺼내고
        //1-3-2. 아직 저장되지 않은 key라면 -> value로 새 배열을 선언하고, ex){headgear: ["yellow_hat", ...]}
        const list = map.get(clothType) ?? new Array(); // {headgear: []}
        
        //1-3-2. 얻어낸 value 배열에 해당 타입의 옷을 넣어준다. 
        list.push(clothes[i][0]) // {headgear: ["yellow_hat", ...]}
        
        //1-3-2. 방금 업데이트 한 해당 종류의 vlaue를 다시 해쉬맵에 set 해준다.
